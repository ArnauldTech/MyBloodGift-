import { ChevronDown, ChevronUp, HeartPulse, type LucideIcon } from "lucide-react"
...
function Card({ title, description, className, color }: CardsProps) {
  return (
    <div
      className={`rounded-xl shadow-md hover:shadow-lg ${color || "bg-white"} flex flex-col justify-center items-center gap-4 p-6 md:p-8 h-48 md:h-56 w-full transition-shadow duration-300`}
    >
      {title && <h3 className="text-red-600 text-lg md:text-xl font-bold text-center">{title}</h3>}
...
export function EmergencyCard({ className, title, status, information }: CardProps) {
  return (
    <div
      className={`${className} bg-linear-to-br from-red-500 to-red-700 rounded-lg px-6 py-6 md:p-8 flex flex-col gap-20 md:gap-12 transition-transform duration-300 cursor-pointer hover:shadow-xl hover:scale-105`}
    >
      <div className="flex items-start justify-between gap-4">
        ...
        {status && (
          <div className="bg-white/20 backdrop-blur-md rounded-full px-3 md:px-4 py-1 text-xs md:text-sm font-semibold text-white whitespace-nowrap">
            {status}
          </div>
        )}
      </div>
...
export function IllustrativeCard({ className, title, description }: CardProps) {
  return (
    <div
      className={`${className} rounded-lg relative overflow-hidden cursor-pointer group transition-transform duration-300 hover:shadow-xl`}
    >
      <div className="absolute inset-0 bg-linear-to-br from-blue-950 via-blue-800 to-red-700" />
      <div className="flex flex-col gap-3 p-4 md:p-6 items-start absolute bottom-0 w-full">
        <div className="flex items-center gap-2">
          <HeartPulse className="h-7 w-7 text-red-200" aria-hidden="true" />
...
export function DescriptiveCard({
  ...
  return (
    <div
      className={` relative ${className} rounded-xl shadow-md hover:shadow-lg ${color || "bg-white"} flex flex-col justify-between gap-2 md:gap-4 p-6 md:p-8 h-48 md:h-56 w-full transition-shadow duration-300`}
    >
      <div
        className={`absolute ${position || "top-5 right-5"} flex h-12 w-12 items-center justify-center rounded-2xl bg-red-50 text-red-600`}
      >
        <Icon className="h-6 w-6" aria-hidden="true" />
      </div>
      <div className="w-full mt-[20%] sm:mt-[17%] md:mt-[15%]">
        {title && <h3 className="text-red-600 text-lg md:text-xl font-bold  ">{title}</h3>}
...
export function CategorieCard({ className, children, categorie }: categorieCard) {
  return (
    <div
      className={`${className} flex gap-2 rounded-lg p-2 border boder-primary outline-1 outline-primary bg-white hover:bg-primary/50 `}
    >
      {children}
